...
int main(int argc, const char **argv) {
    @autoreleasepool {
        ...
        // --- CPU baseline timing (built-in / and %)
        double t0 = now_sec();
        for (uint32_t i = 0; i < N; ++i) {
            q_cpu[i] = xs[i] / ys[i];
            r_cpu[i] = xs[i] % ys[i];
        ...
        NSString *mslPath = @"gpu_divider.metal";
        ...
        printf("GPU (Metal):  %.3f s  (%.2f Mops/s)  sinksums: %" PRIu64 ", %" PRIu64 "\\n",
               gpu_s, gpu_ops/1e6, gq, gr);
        printf("Compare: mismatches = %zu (of %u)\\n", mism, N);
        printf("Speedup (GPU/CPU): %.2fx\\n", (gpu_s > 0 ? cpu_s / gpu_s : 0.0));
