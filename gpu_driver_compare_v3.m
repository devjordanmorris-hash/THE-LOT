...
static void stats(const char* tag, double secs, uint32_t N, uint64_t sinkQ, uint64_t sinkR){
    ...
    printf("%-16s %.3f s  (%.2f Mops/s)  sinksums: %" PRIu64 ", %" PRIu64 "\n", tag, secs, mps, sinkQ, sinkR);
}

// Plain C helper (no lambdas) to run a compute kernel and compare against CPU
...
int main(int argc, const char **argv) {
    @autoreleasepool {
        ...
        NSString *mslPath = @"gpu_divider_compare_v3.metal";
        ...
        id<MTLFunction> f_ours    = [lib newFunctionWithName:@"div64_evenfirst_restoring"];
        ...
        id<MTLComputePipelineState> p_ours = [device newComputePipelineStateWithFunction:f_ours error:&err];
        if (!p_ours) { fprintf(stderr, "Pipeline ours error: %s\n", err.localizedDescription.UTF8String); return 1; }
        ...
        run_kernel_compare(p_ours, q, bx, by, bq, br, bn, q_cpu, r_cpu, N, "GPU ours", &sq2, &sr2, &s2);
        stats("GPU ours:", s2, N, sq2, sr2);

        printf("Speedup GPU_ours / CPU: %.2fx\n", (s2 > 0 ? ( (double)N / s2 ) / ( (double)N / cpu_secs ) : 0.0));
        printf("Speedup GPU_ours / GPU_builtin: %.2fx\n", (s2 > 0 ? s1 / s2 : 0.0));
