...
int main(int argc, const char **argv) {
    @autoreleasepool {
        ...
        uint32_t THRESH = 61u; // default: ~12.5% to GPU for uniform64
        int mixed = 0;         // 0=uniform64, 1=mixed50 (small<=32, big>=56)
        ...
        if (cpu_threads <= 0) cpu_threads = active_cpu_count(); // auto

        printf("Hybrid: THRESH<=%u -> GPU | dataset=%s | CPU_threads=%d\n",
               THRESH, mixed ? "mixed50" : "uniform64", cpu_threads);
        ...
        if (!mixed){
            ...
            for (uint32_t i = 0; i < N; ++i) {
                ...
                uint64_t y;
                if ((i & 1u) == 0) {
                    do { y = xrng() & ((1ULL<<32) - 1ULL); } while (y == 0); // small
                } else {
                    y = xrng() | (1ULL<<55); if (y == 0) y = 1;               // big
                }
                xs[i] = x; ys[i] = y;
            }
        }

        // Reference CPU baseline (single-thread) to validate
        double t0 = now_sec();
        ...
        NSError *err = nil;
        NSString *mslPath = @"gpu_divider_hybrid_v7.metal";
        ...
        // --- Launch GPU and CPU in parallel ---
        __block double gpu_secs = 0.0;
        __block uint64_t sinkQ_gpu=0, sinkR_gpu=0;
        __block double cpu_secs = 0.0;
        __block uint64_t sinkQ_cpu=0, sinkR_cpu=0;
        ...
        // CPU task (parallel over big subset)
        dispatch_group_async(group, q_cpu, ^{
            double c0 = now_sec();
            // Split into chunks and run with dispatch_apply
            uint32_t T = (uint32_t)cpu_threads;
            ...
            dispatch_apply(T, q_cpu, ^(size_t t){
                ...
                // combine sinksums (not atomic, but low contention if we just xor after)
        ...
        for (uint32_t i = 0; i < N; ++i) {
            ...
            if (q_out[i] != q_ref[i] || r_out[i] != r_ref[i]) mism++;
        }
        ...
        double hybrid_secs = (gpu_secs > cpu_secs ? gpu_secs : cpu_secs); // overlapped
