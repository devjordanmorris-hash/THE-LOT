// jlo3_portable.c  — J-LO triangle engine + zlib-only benchmarks (no Compression.framework)
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <stdint.h>
...
static void make_signal(float *x){
    ...
    for(int i=0;i<N;i++){
        ...
        double v=sin(2.0*M_PI*f1*t)*0.7 + 0.25*sin(2.0*M_PI*f2*t+0.6);
        x[i]=(float)v;
    }
}
...
static void rms_and_max(const float *a, const float *b, double *rms, double *maxabs){
    double s=0, m=0;
    for(int i=0;i<N;i++){ double e=(double)a[i]-b[i]; s+=e*e; if (fabs(e)>m) m=fabs(e); }
...
static void compJLO_fast(const int32_t *a, int32_t eps, int step,
    ...
    int sp=0, sc=0;
    stack[sp++] = (Segment){0,a[0],N-1,a[N-1]};
    while(sp>0){
        Segment s=stack[--sp];
        int i0=s.i0, i1=s.i1, n=i1-i0;
        int32_t v0=s.v0, v1=s.v1;
        if(n<=1){ segs[sc++]=s; continue; }
        int idx; int64_t err;
        maxDev_step(a,i0,v0,i1,v1, step, &idx,&err);
        if(err <= (int64_t)eps * n){
            segs[sc++]=s;
        }else{
            int32_t vm=a[idx];
            stack[sp++]=(Segment){idx,vm,i1,v1};
            stack[sp++]=(Segment){i0,v0,idx,vm};
        }
    }
...
static void reconJLO(const Segment *segs, int count, int32_t *y){
    for(int s=0;s<count;s++){
        int i0=segs[s].i0, i1=segs[s].i1, n=i1-i0;
        int32_t v0=segs[s].v0, v1=segs[s].v1;
        if(n<=0){ if(i0>=0 && i0<N) y[i0]=v0; continue; }
        int64_t dv=(int64_t)v1-(int64_t)v0;
        for(int i=i0;i<=i1;i++){
            int64_t t=i-i0;
            y[i]=(int32_t)((int64_t)v0 + (dv*t)/n);
        }
    }
}
...
static void bench_zlib(const uint8_t *input, size_t len){
    ...
    if(rc!=Z_OK){ printf("zlib compress failed (%d)\n", rc); free(dst); return; }

    uint8_t *dec=(uint8_t*)malloc(len);
    t0=now_ms();
    uLongf decLen = (uLong)len;
    rc = uncompress(dec,&decLen,dst,outSize);
    double decT=now_ms()-t0;

    if(rc!=Z_OK || decLen!=(uLong)len){ printf("zlib decompress failed\n"); }
...
int main(void){
    ...
    bench_zlib(bytes, raw);

    // J-LO config
    int32_t *xFix=(int32_t*)malloc(sizeof(int32_t)*N);
    float_to_fix(x, xFix);
    int32_t eps=(int32_t)(1e-4 * SCALE);
    int32_t dq =(int32_t)(1e-5 * SCALE);
    int step = 4; // try 4 or 8 for speed; 1 = max precision
    ...
    // ---- build a payload: [N, eps, dq, segCount] + segs + residuals ----
    size_t header_sz = 4*sizeof(int32_t);
    size_t seg_sz    = segCount * 4 * sizeof(int32_t); // i0,v0,i1,v1
    size_t q_sz      = N * sizeof(int16_t);
    size_t pay_sz    = header_sz + seg_sz + q_sz;

    uint8_t *payload = (uint8_t*)malloc(pay_sz);
    uint8_t *p = payload;

    // header: N, eps, dq, segCount (32-bit)
    int32_t hdr[4] = { N, eps, dq, segCount };
    memcpy(p, hdr, header_sz); p += header_sz;

    // triangles
    memcpy(p, segs, seg_sz); p += seg_sz;
